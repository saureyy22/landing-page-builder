...
const UndoRedoControls: React.FC = () => {
  ...
  const historyPosition = history.past.length;
  const totalHistoryLength = history.past.length + 1 + history.future.length;
    ...
    <div className="undo-redo-controls">
      <div className="undo-redo-buttons">
        ...
        <button
          ...
          title={`Redo (Ctrl+Y) - ${history.future.length} actions available`}
          aria-label={`Redo next action. ${history.future.length} redo actions available.`}
        >
          <svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor">
